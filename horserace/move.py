...
def main():
    url_move_1 = 'https://db.netkeiba.com/?pid=race_list&word=%C3%E6%BB%B3%CC%C6%C7%CF&range='

    goal = []
    goal = get_url(url_move_1, goal, '中山牝馬S(G3)')
    #goal = get_url(url_move_2, goal, '有馬記念(G1)')

    final = race_result(8,goal,'中山牝馬S(G3)')
    return final
